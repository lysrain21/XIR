...
import {
    ILayerZeroPriceFeed
} from "@layerzerolabs/lz-evm-messagelib-v2/contracts/interfaces/ILayerZeroPriceFeed.sol";
...
contract DeployLayerZeroNative {
    ...
    function run() external {
        ...
        uint32[] memory remoteEids = _remoteEids(localEid);
        ...
        PriceFeed priceFeed = PriceFeed(
            address(
                new ERC1967Proxy(
                    ...
                    abi.encodeCall(PriceFeed.initialize, (deployer))
                )
            )
        );
        priceFeed.setNativeTokenPriceUSD(1e20);
        ILayerZeroPriceFeed.UpdatePrice[] memory prices =
            new ILayerZeroPriceFeed.UpdatePrice[](remoteEids.length);
        for (uint256 i = 0; i < remoteEids.length; i++) {
            prices[i] = ILayerZeroPriceFeed.UpdatePrice(
                remoteEids[i],
                ILayerZeroPriceFeed.Price(1e20, 1, 1)
            );
        }
        ...
        IDVN.DstConfigParam[] memory dvnConfigs =
            new IDVN.DstConfigParam[](remoteEids.length);
        IExecutor.DstConfigParam[] memory executorConfigs =
            new IExecutor.DstConfigParam[](remoteEids.length);
        SetDefaultUlnConfigParam[] memory ulnConfigs =
            new SetDefaultUlnConfigParam[](remoteEids.length);
        ...
        for (uint256 i = 0; i < remoteEids.length; i++) {
            ...
            ulnConfigs[i] = SetDefaultUlnConfigParam(
                remoteEids[i],
                UlnConfig(1, 1, 0, 0, requiredDvns, new address[](0))
            );
            sendExecutorConfigs[i] = SetDefaultExecutorConfigParam(
                remoteEids[i],
                ExecutorConfig(1000, address(executor))
            );
        }
    ...
    function _remoteEids(uint32 localEid) private pure returns (uint32[] memory result) {
        uint32[3] memory all = [uint32(49001), uint32(49002), uint32(49003)];
        result = new uint32[](2);
        uint256 cursor;
        for (uint256 i = 0; i < all.length; i++) {
            if (all[i] != localEid) result[cursor++] = all[i];
        }
        require(cursor == 2, "unsupported local eid");
    }
