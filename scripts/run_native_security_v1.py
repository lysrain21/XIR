...
from xir_lab.native.security_v1 import NativeSecurityCampaign


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Run benign negative-input conformance on an isolated native stack."
    )
    ...
    parser.add_argument(
        "--repetition-limit",
        type=int,
        help="smoke-only limit; omit for the frozen 30 repetitions per route/case",
    )
    parser.add_argument(
        "--attempt-namespace",
        help="smoke-only identity namespace; omit for the frozen full campaign",
    )
    ...
    runner = NativeExperimentRunner(
        ...
        root_signer_private_key=args.root_signer_key_file.read_text(
            encoding="utf-8"
        ).strip(),
    ...
    campaign = NativeSecurityCampaign(
        runner=runner,
        config_path=args.config,
        deployment_path=args.deployment,
        deployer_private_key=args.deployer_key_file.read_text(
            encoding="utf-8"
        ).strip(),
